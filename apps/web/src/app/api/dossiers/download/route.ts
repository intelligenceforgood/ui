...
export async function GET(request: NextRequest) {
  ...
  if (!isWithinBase(baseDir, resolvedPath)) {
    return NextResponse.json({ error: "Requested path outside allowed base" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Unable to read dossier artifact" }, { status: 404 });
  }
}
