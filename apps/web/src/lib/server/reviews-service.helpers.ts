...
function extractSavedSearchDescriptor(
  ...
  const id = pickFirstString(sources, [
    "saved_search_id",
    "savedSearchId",
    "search_id",
  ]);
  const name = pickFirstString(sources, [
    "saved_search_name",
    "savedSearchName",
  ]);
  const owner = pickFirstString(sources, [
    "saved_search_owner",
    "savedSearchOwner",
    "owner",
  ]);
...
export function mapHistoryEvent(
  ...
  const actor =
    typeof payload.actor === "string" && payload.actor.length
      ? payload.actor
      : "analyst";
  const queryValue =
    toNonEmptyString(requestPayload.query) ??
    toNonEmptyString(requestPayload.text);
  ...
  const classificationValue = taxonomyValues.length
    ? taxonomyValues[0]
    : undefined;
  ...
  return {
    ...
    caseId:
      typeof rawPayload.case_id === "string" ? rawPayload.case_id : undefined,
    resultCount:
      typeof rawPayload.results_count === "number"
        ? rawPayload.results_count
        : undefined,
...
export function mapSavedSearch(
  payload: Record<string, unknown>,
): SavedSearchRecord {
...
export function mapHybridSearchSchemaPayload(
  value: Record<string, unknown>,
): HybridSearchSchema {
  return {
    indicatorTypes: toStringArray(
      value.indicator_types ?? value.indicatorTypes,
    ),
    ...
    entityExamples: mapEntityExamples(
      value.entity_examples ?? value.entityExamples,
    ),
