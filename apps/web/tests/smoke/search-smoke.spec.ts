...
test.describe("search smoke", () => {
  test("supports filters, entity builder, and saved searches", async ({ page }) => {
    ...
    const applyEntityButton = page.getByRole("button", { name: /Apply entity filters/i });
    await expect(applyEntityButton).toBeEnabled();
    await applyEntityButton.click();
    await expect(page.getByText(/1 entity filter/i)).toBeVisible();
    ...
    await expect(page.getByText(/Dataset: network_smoke/i).first()).toBeVisible();
    ...
    await expect(page.getByText(new RegExp(`Saved "${savedSearchName}"`, "i"))).toBeVisible();
  });
});
